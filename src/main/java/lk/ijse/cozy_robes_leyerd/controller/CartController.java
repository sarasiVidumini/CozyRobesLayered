...
//    private final OrderDetailsModel orderDetailsModel = new OrderDetailsModel();
    ...
    private CartBO cartBO = (CartBO) BOFactory.getInstance().getBO(BOFactory.BOTypes.CART);
    ...
    public void loadNextId() {
        try {
            String nextId = cartBO.getNextOrderId();
    ...
    public void loadProductDetails(String productId){
        try {
            ProductDTO product = cartBO.getProductByIds(productId);
            if (product != null) {
    ...
    public void searchCustomerContact(KeyEvent keyEvent) {
        ...
        try {
            ...
            String customerId = cartBO.getCustomerIdByContact(contact);
            if (customerId != null) {
                lblCustomerName.setText(cartBO.getCustomerNameById(customerId));
            }else {
    ...
    public void loadCustomerIds(){
        try {
            ArrayList<String> customerIdList = cartBO.getAllCustomerIds();
    ...
    public void loadProductIds() {
        try {
            ArrayList<String> productIdsList =  cartBO.getAllProductIds();
    ...
    public void btnPlaceOrderOnAction(ActionEvent actionEvent) {
        ...
        try {
            ...
            String customerId = cartBO.getCustomerIdByContact(customerContact);
            String status = "Completed";

            String paymentId = cartBO.getNextPaymentId();
            ...
            boolean orderSaved = cartBO.saveNewOrder(orderId, customerId, orderDate, status, productId
            );
            ...
            for (CartTM cartTM : cartData) {
                String orderDetailId = cartBO.getNextOrderDetailsId();
                boolean productSaved = cartBO.saveNewOrderDetails(
                        orderDetailId,
                        orderId,
                        cartTM.getProductId(),
                        cartTM.getCartQty(),
                ...
                boolean productUpdated = cartBO.reduceQty(
                        cartTM.getProductId(),
                        cartTM.getCartQty()
                );
            ...
            boolean paymentSaved = cartBO.savePayment(new PaymentDTO(paymentId, orderId, paymentMethod, totalAmount));
    ...
    private void refreshPage() throws SQLException {
        lblOrderId.setText(cartBO.getNextOrderId());
