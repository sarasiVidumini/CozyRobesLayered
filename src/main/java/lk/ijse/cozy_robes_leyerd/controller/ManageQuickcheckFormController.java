...
import lk.ijse.cozy_robes_leyerd.viewTm.QuickcheckTM;
import lk.ijse.cozy_robes_leyerd.dao.Impl.QuickcheckDAOImpl;
...
public class ManageQuickcheckFormController implements Initializable {

    public AnchorPane ancQuickcheckPage;
//    private final QuickcheckModel quickcheckModel = new QuickcheckModel();
//    private QuickcheckDAOImpl quickcheckDAO = new QuickcheckDAOImpl();

    private QuickcheckBO quickcheckBO = (QuickcheckBO) BOFactory.getInstance().getBO(BOFactory.BOTypes.QUICKCHECK);
    ...
    public void btnDeleteOnAction(ActionEvent actionEvent) {
        ...
        if (result.isPresent() && result.get() == ButtonType.YES) {
            ...
            try {
                boolean isDeleted = quickcheckBO.delete(checkId);
                if (isDeleted) {
    ...
    public void search(KeyEvent keyEvent) {
        ...
        if (search.isEmpty()) {
            ...
            try {
                ArrayList<QuickcheckDTO> checkList = quickcheckBO.search(search);
                tblQuickcheck.setItems(FXCollections.observableArrayList(
                        checkList.stream()
                                .map(quickcheckDto -> new QuickcheckTM(
                                        quickcheckDto.getCheckId(),
