...
//            loadOrderIds();
    ...
    public void btnUpdateOnAction(ActionEvent actionEvent) {
        ...
        try {
            boolean isUpdated = paymentBO.update(new PaymentDTO(paymentId,orderId,paymentMethod,totalAmount));
            if (isUpdated) {
    ...
    public void btnSaveOnAction(ActionEvent actionEvent) {
        ...
        if (paymentId.isEmpty() || totalAmount==0.0) {
            new Alert(Alert.AlertType.ERROR, "Please fill out all fields!").show();
            return;
        }
        ...
        try {
            boolean isSaved = paymentBO.save(new PaymentDTO(paymentId,orderId,paymentMethod,totalAmount));
            if (isSaved) {
    ...
    public void onClickedTable(MouseEvent mouseEvent) {
        ...
        if (selectedItem != null) {
            ...
            txtOrders.setText(selectedItem.getOrderId());
...
//    }
    ...
    public void search(KeyEvent keyEvent) {
        ...
        if (search.isEmpty()) {
            ...
            try {
                ArrayList<PaymentDTO> paymentList = paymentBO.search(search);
                tblPayment.setItems(FXCollections.observableArrayList(
                        paymentList.stream()
                                .map(paymentDto -> new PaymentTM(
                                        paymentDto.getPaymentId(),
